...
public class ListenersImplements implements ITestListener,ISuiteListener {
	...
	public void onTestStart(ITestResult result) {
		...
		extents.set(test);
		extents.get().log(Status.INFO, name+"----->Execution Starts");
	}

	@Override
	public void onTestSuccess(ITestResult result) {
		String methodname=result.getMethod().getMethodName();
		extents.get().log(Status.PASS,methodname+"---->Passed");
	}
	...
	public void onTestFailure(ITestResult result) {
		...
		extents.get().log(Status.FAIL, result.getThrowable());//D
		extents.get().log(Status.FAIL, mname+"----> Failed");
	}

	@Override
	public void onTestSkipped(ITestResult result) {
		String mname=result.getMethod().getMethodName();
		extents.get().skip("test skipped");
	}

	
	@Override
	public void onFinish(ITestContext context) {
		extents.get().log(Status.INFO, "Test Completed");
	}
